...
def write_external_id_file(all_users: Clone, username: str, account_id: str) -> None:
    (all_users / sha1sum("gerrit:" + username)).write_text("\n".join([
        "[externalId \"gerrit:" + username + "\"]",
...
def add_account_external_id(all_users: Clone, username: str, account_id: str) -> None:
    ...
    write_external_id_file(all_users, username, account_id)
    git(all_users, ["add", "."])
    commit_and_push(
...
def create_admin_user(email: Email, pubkey: PubKey, all_users_url: Url) -> None:
    ...
    if not try_action(lambda: fetch(all_users, admin_ref)):
        ...
        write_external_id_file(all_users, "admin", "1")
