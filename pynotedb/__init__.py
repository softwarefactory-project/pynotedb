...
def commit_and_push(repo: Clone, message: str, ref: Ref) -> None:
    try_action(lambda: git(repo, ["commit", "-a", "-m", message]))
    # TODO: check if ref is already pushed
    try_action(lambda: git(repo, ["push", "origin", "HEAD:" + ref]))
...
def create_admin_user(email: Email, pubkey: PubKey, all_users_url: Url) -> None:
    ...
    if not try_action(lambda: fetch(all_users, admin_ref)):
        ...
        (all_users / sha1sum("username:admin")).write_text("\n".join([
...
def main() -> None:
    ...
    def usage(argv: List[str]) -> argparse.Namespace:
        ...
        parser.add_argument("--pubkey", description="SSH public key content")
        parser.add_argument("--all-users", description="URL of the All-Users project")
        parser.add_argument("--fqdn", description="The FDQN for admin email")
