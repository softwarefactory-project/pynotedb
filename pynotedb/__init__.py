...
def main() -> None:
    ...
    def usage(argv: List[str]) -> argparse.Namespace:
        ...
        parser.add_argument("--scheme", help='the auth scheme to use when creating the admin user. Supported schemes are "gerrit" and "keycloak-oauth"')
...
def main_do(args: argparse.Namespace) -> None:
    if args.action == "create-admin-user":
        ...
        if not args.scheme:
            scheme = ExternalScheme('gerrit')
        elif args.scheme in ['gerrit', 'keycloak-oauth']:
            scheme = ExternalScheme(args.scheme)
        else:
            raise RuntimeError('valid schemes are "gerrit" or "keycloak-oauth"')
